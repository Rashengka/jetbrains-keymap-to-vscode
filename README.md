...
| `--unkeep-key KEY` | Take a key off that list again (`recommended` turns off `--keep-recommended`) |
| `--keep-recommended` | Leave VS Code's documented default shortcuts to VS Code (remembered) |
...
`--keep-recommended` does the same for every key that VS Code documents as a
default shortcut
([default keybindings](https://code.visualstudio.com/docs/reference/default-keybindings))
when the generated shortcut would run a different command there. By default
everything you changed in JetBrains is transferred; this switch is the careful
variant. It is remembered on the same line; `--unkeep-key recommended` turns it
off. The list comes from `internal/convert/vscode-defaults.json`, generated by
`go run ./tools/gen-vscode-defaults`.
